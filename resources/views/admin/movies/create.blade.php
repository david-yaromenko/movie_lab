...
<div class="max-w-xl mx-auto bg-white rounded-2xl shadow p-8 mt-10">
    ...
    <h1 class="text-2xl font-bold mb-6 text-gray-800">Додати фільм</h1>
    ...
    <form action="{{ route('admin.movies.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title[uk]" value="{{ old('title.uk') }}"
                   class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
        </div>
        ...
        <div>
            ...
            <textarea name="description[en]" rows="3"
                      class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description.en') }}</textarea>
        </div>

        <div>
            <label class="block text-gray-700 font-semibold mb-2">Опис (UK)</label>
            <textarea name="description[uk]" rows="3"
                      class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description.uk') }}</textarea>
        </div>
...
<div class="space-y-4">
    ...
    {{-- Режисери --}}
    <div>
        <label class="block text-gray-700 font-semibold mb-2">Режисери</label>
        <select name="roles[director][]" multiple
                class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            @foreach($persons as $person)
                @if($person->types->contains('type', 'director'))
                    <option value="{{ $person->id }}"
                        {{ in_array($person->id, old('roles.director', [])) ? 'selected' : '' }}>
                        {{ $person->name['en'] ?? $person->name['uk'] ?? 'Без имени' }}
                    </option>
                @endif
            @endforeach
        </select>
        <small class="text-gray-500">Утримуйте Ctrl (Windows) або Cmd (Mac), щоб вибрати кількох</small>
    </div>

    {{-- Сценаристи --}}
    <div>
        <label class="block text-gray-700 font-semibold mb-2">Сценаристи</label>
        <select name="roles[screenwriter][]" multiple
                class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            @foreach($persons as $person)
                @if($person->types->contains('type', 'screenwriter'))
                    <option value="{{ $person->id }}"
                        {{ in_array($person->id, old('roles.screenwriter', [])) ? 'selected' : '' }}>
                        {{ $person->name['en'] ?? $person->name['uk'] ?? 'Без имени' }}
                    </option>
                @endif
            @endforeach
    ...
    {{-- Актори --}}
    <div>
        <label class="block text-gray-700 font-semibold mb-2">Актори</label>
        <select name="roles[actor][]" multiple
                class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            @foreach($persons as $person)
                @if($person->types->contains('type', 'actor'))
                    <option value="{{ $person->id }}"
                        {{ in_array($person->id, old('roles.actor', [])) ? 'selected' : '' }}>
                        {{ $person->name['en'] ?? $person->name['uk'] ?? 'Без имени' }}
                    </option>
                @endif
            @endforeach
    ...
    {{--Композитори --}}
    <div>
        <label class="block text-gray-700 font-semibold mb-2">Композитори</label>
        <select name="roles[composer][]" multiple
                class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            @foreach($persons as $person)
                @if($person->types->contains('type', 'composer'))
                    <option value="{{ $person->id }}"
                        {{ in_array($person->id, old('roles.composer', [])) ? 'selected' : '' }}>
                        {{ $person->name['en'] ?? $person->name['uk'] ?? 'Без имени' }}
                    </option>
                @endif
            @endforeach
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="datetime-local" name="watch_start_date" value="{{ old('watch_start_date') }}"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            <div>
                <label class="block text-gray-700 font-semibold mb-2">Дата закінчення перегляду</label>
                <input type="datetime-local" name="watch_end_date" value="{{ old('watch_end_date') }}"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="trailer_id_youtube" value="{{ old('trailer_id_youtube') }}"
